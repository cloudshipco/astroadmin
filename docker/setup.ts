 ...
 * 2. Asks about deployment mode (direct vs behind proxy)
 * 3. Generates SSH deploy keys
 * 4. Clones the site repos
 * 5. Creates .env with credentials
 * 6. Generates appropriate nginx configs
 */
...
async function promptChoice(rl: ReturnType<typeof createPrompt>, question: string, choices: string[]): Promise<number> {
  console.log(question);
  choices.forEach((choice, i) => {
    console.log(`  ${colors.bold}${i + 1}${colors.reset}. ${choice}`);
  });
  const answer = await rl.question(`Choice ${colors.dim}[1-${choices.length}]${colors.reset}: `);
  const num = parseInt(answer.trim()) || 1;
  return Math.max(1, Math.min(choices.length, num)) - 1;
}
...
function createHostNginxConfig(config: {
  ...
  const { mainDomain, adminDomain, previewDomain, proxyPort } = config;

  const content = `# AstroAdmin Host nginx Configuration
# Copy this to /etc/nginx/sites-available/ and symlink to sites-enabled
#
# After copying:
#   sudo ln -s /etc/nginx/sites-available/astroadmin.conf /etc/nginx/sites-enabled/
#   sudo nginx -t
#   sudo systemctl reload nginx

# All three domains proxy to Docker container
server {
    listen 80;
    server_name ${mainDomain} www.${mainDomain} ${adminDomain} ${previewDomain};
    ...
    location / {
        ...
        # WebSocket support (for HMR and live updates)
...
# After setting up SSL with certbot, it will modify this file to add:
# - listen 443 ssl
# - SSL certificate paths
# - HTTP to HTTPS redirect
`;

  const configPath = join(DOCKER_DIR, "host-nginx.conf");
  writeFileSync(configPath, content);
  success(`Created host-nginx.conf for your server's nginx`);
}
...
function createEnvFile(config: {
  ...
  const portConfig = config.proxyMode
    ? `# Proxy mode: Docker listens on localhost only
DOCKER_PORT=${config.proxyPort}
DOCKER_BIND=127.0.0.1`
    : `# Direct mode: Docker listens on all interfaces
HTTP_PORT=80
HTTPS_PORT=443`;
...
function updateDockerComposeForProxy(port: number): void {
  const composePath = join(DOCKER_DIR, "docker-compose.yml");
  let content = readFileSync(composePath, "utf-8");

  // Replace the nginx ports section
  content = content.replace(
    /ports:\n\s+- "\$\{HTTP_PORT:-80\}:80"\n\s+- "\$\{HTTPS_PORT:-443\}:443"/,
    `ports:\n      - "\${DOCKER_BIND:-127.0.0.1}:\${DOCKER_PORT:-${port}}:80"`
  );

  writeFileSync(composePath, content);
  success(`Updated docker-compose.yml for proxy mode (port ${port})`);
}
...
async function main() {
  ...
  try {
    ...
    log("How will you deploy this?");
    const deployMode = await promptChoice(rl, "", [
      "Behind host proxy (recommended) - Host nginx/caddy handles SSL and proxies to Docker",
      "Direct - Docker nginx listens on port 80/443 directly",
    ]);
    const proxyMode = deployMode === 0;
    const proxyPort = 8080;

    // Protocol (only ask for direct mode, proxy mode will use host's SSL)
    let useHttps = true;
    if (!proxyMode) {
      useHttps = await confirm(rl, "Will you be using HTTPS?", true);
    }
    ...
    console.log(`  Deploy mode:    ${proxyMode ? `Behind host proxy (port ${proxyPort})` : "Direct (port 80/443)"}`);
